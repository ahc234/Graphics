...
public class SceneP1 extends JFrame implements GLSceneDrawer, ChangeListener {
	...
	public void draw(GLAutoDrawable drawable, CameraController cameraController) {
		...
		programToUse.setModelView(gl, cameraController.getModelView());
		
		// set material
		programToUse.setMaterial(gl, material);
		
		// set lighting
		setLights(gl, programToUse, cameraController.getModelView());

		if (sceneViewPanel.isWireframeMode())
		{
			meshes.get(meshIndex).drawWireframe(gl);
		}
		else
		{
			meshes.get(meshIndex).draw(gl);
		}
		Program.unuse(gl);
	}
